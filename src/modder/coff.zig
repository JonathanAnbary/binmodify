...
pub const Modder: type = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, parsed_source: *const coff.Coff, parse_source: anytype) !Self {
        _ = parse_source;
        const sechdrs = try parsed_source.getSectionHeadersAlloc(gpa);
        ...
        const optional_header = parsed_source.getOptionalHeader();

        return Self{
            .header = .{
                .file_alignment = switch (optional_header.magic) {
                    coff.IMAGE_NT_OPTIONAL_HDR64_MAGIC => parsed_source.getOptionalHeader64().file_alignment,
                    coff.IMAGE_NT_OPTIONAL_HDR32_MAGIC => parsed_source.getOptionalHeader32().file_alignment,
                    else => return Error.InvalidOptionalHeaderMagic,
                },
                .image_base = parsed_source.getImageBase(),
                .coff_header_offset = parsed_source.coff_header_offset,
                .size_of_optional_header = parsed_source.getCoffHeader().size_of_optional_header,
            },
            .sechdrs = sechdrs,
    ...
    pub fn get_cave_option(self: *const Self, wanted_size: u64, flags: common.FileRangeFlags) !?SecEdge {
    ...
    fn set_sechdr_field(self: *Self, index: usize, val: u64, comptime field_name: []const u8, parse_source: anytype) !void {
    ...
    pub fn create_cave(self: *Self, size: u64, edge: SecEdge, parse_source: anytype) !void {
